 ...
 */
struct A4_ATN {
    /// Pool that's used to allocate and manage all objects in the ATN.
    ...
    struct {
        struct A4_ATNDecisionState* data;
        size_t size;
    } decision_states;

    /// Maps from rule index to starting state number.
    struct A4_ATNRuleStartState* rule_to_start_state;

    /// Maps from rule index to stop state number.
    struct A4_ATNRuleStopState* rule_to_stop_state;
