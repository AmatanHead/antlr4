...
A4_ERRNO A4_ATN_Load(A4_ATN** result, A4_MemoryPool* pool, const uint16_t* bc_raw, size_t bc_size) {
    ...
    *result = NULL;
    ...
    READ(1);
    if (*bco != 3) ERR(A4_ATN_BC_NOT_SUPPORTED);

    READ(8);
    A4_UUID ver = A4_UUID_ReadBinary(bco);
    if (A4_ATN_FeatureIndex(ver, SUPPORTED_UUIDS) == ~((size_t) 0)) ERR(A4_ATN_BC_NOT_SUPPORTED);
    ...
    if (atn->grammar_type > A4_ATNT_MAX_VAL) ERR(A4_ATN_BC_INVALID);
    ...
    for (size_t i = 0; i < n_states; ++i) {
        ...
        if (type > A4_ATNST_MAX_VAL) ERR(A4_ATN_BC_INVALID);
    ...
    for (size_t i = 0; i < num_non_greedy_states; ++i) {
        READ(1);
        uint16_t st = *bco;
        if (st >= atn->states.size) ERR(A4_ATN_BC_INVALID);
        if (!A4_IsDecisionState(atn->states.data[st])) ERR(A4_ATN_BC_INVALID);
    ...
    for (size_t i = 0; i < num_precedence_states; ++i) {
        ...
        uint16_t st = *bco;
        if (st >= atn->states.size) ERR(A4_ATN_BC_INVALID);
        if (!A4_IsRuleStartState(atn->states.data[st])) ERR(A4_ATN_BC_INVALID);
    ...
    if (!atn->rule_to_stop_state) ERR(ENOMEM);
    ...
    if (!atn->mode_to_start_state.data) ERR(ENOMEM);

    for (size_t i = 0; i < num_modes; ++i) {
        READ(1);
        uint16_t st = *bco;
        if (st >= atn->states.size) ERR(A4_ATN_BC_INVALID);
        if (!A4_IsTokenStartState(atn->states.data[st])) ERR(A4_ATN_BC_INVALID);
    ...
    if (!state_to_transitions) ERR(ENOMEM);

    for (size_t i = 0; i < atn->states.size; ++i) {
        state_to_transitions[i] = TransitionList_New();
        if (!state_to_transitions[i]) ERR(ENOMEM);
    }
    ...
    for (size_t i = 0; i < atn->states.size; ++i) {
        A4_LIST_FOREACH(TransitionList, state_to_transitions[i], it) {
            ...
            if (!return_transition) ERR(ENOMEM);
    ...
    for (size_t i = 0; i < atn->states.size; ++i) {
        ...
        if (state_to_transitions[i]) {
            ...
            if (!state->transitions.data) ERR(ENOMEM);
    ...
    for (size_t i = 0; i < atn->states.size; ++i) {
        if (A4_IsBlockStartStateBase(atn->states.data[i])) {
            ...
            if (state->end_state == NULL) ERR(A4_ATN_BC_INVALID);
            if (state->end_state->start_state != NULL) ERR(A4_ATN_BC_INVALID);
    ...
    if (!atn->decision_to_state.data) ERR(ENOMEM);

    for (uint16_t i = 0; i < num_decisions; ++i) {
        READ(1);
        uint16_t sn = *bco;
        if (sn >= atn->states.size) ERR(A4_ATN_BC_INVALID);
        if (!A4_IsDecisionState(atn->states.data[sn])) ERR(A4_ATN_BC_INVALID);
    ...
    if (atn->grammar_type == A4_ATNT_LEXER) {
        ...
        if (!atn->lexer_actions.data) ERR(ENOMEM);

        for (size_t i = 0; i < num_lexer_actions; ++i) {
            READ(1);
            A4_LexerActionType type = *bco;
            if (type > A4_LAT_MAX_VAL) ERR(A4_ATN_BC_INVALID);
            READ(2);
            uint16_t arg1 = *(bco + 0);
            uint16_t arg2 = *(bco + 1);
